...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let entropy = generate_entropy(&pass, 9_000_000);
    ...
    for (wallet_id, item) in wallets.iter().enumerate() {
        ...
        'wallet_roll: for current_id in 1..=to {
            let new_pass: String = format!("{}-{}-{}", mnemonic_init, item, current_id);
            let new_entropy = &generate_entropy(&new_pass, 1000)[0..16];
